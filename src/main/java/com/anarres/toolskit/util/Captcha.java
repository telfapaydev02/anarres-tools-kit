...
import com.anarres.toolskit.support.HashKit;
import com.anarres.toolskit.web.CookieKit;
...
public class Captcha {
	...
	public static void setCaptchaName(String captchaName) {
		if (CharsetKit.isBlank(captchaName)) {
	...
	public static boolean validate(HttpServletRequest request, HttpServletResponse response, String userInputCaptcha) {
		if (CharsetKit.isBlank(userInputCaptcha)) {
			return false;
		}
		userInputCaptcha = userInputCaptcha.toUpperCase();	// 转成大写重要
		userInputCaptcha = HashKit.MD5Encode(userInputCaptcha);
