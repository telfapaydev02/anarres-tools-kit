...
import com.anarres.toolskit.ToolBoxException;
import com.anarres.toolskit.util.Convert;
...
public class BeanKit {
    ...
    public static PropertyDescriptor getPropertyDescriptor(Class<?> clazz, final String fieldName) throws IntrospectionException {
        ...
        for (PropertyDescriptor propertyDescriptor : propertyDescriptors) {
            if (FuncKit.equals(fieldName, propertyDescriptor.getName())) {
                return propertyDescriptor;
            }
        }
        return null;
        ...
        if (isToCamelCase) {
            ...
            for (Entry<?, ?> entry : map.entrySet()) {
                ...
                if (null != key && key instanceof String) {
                    ...
                    map2.put(CharsetKit.toCamelCase(keyStr), entry.getValue());
                } else {
    ...
    public static <T> T fillBeanWithRequestParam(final javax.servlet.ServletRequest request, T bean) {
        final String beanName = CharsetKit.lowerFirst(bean.getClass().getSimpleName());
        return fillBean(bean, new ValueProvider() {
            @Override
            public Object value(String name) {
                String value = request.getParameter(name);
                if (CharsetKit.isEmpty(value)) {
                    // 使用类名前缀尝试查找值
                    value = request.getParameter(beanName + CharsetKit.DOT + name);
                    if (CharsetKit.isEmpty(value)) {
                        // 此处取得的值为空时跳过，包括null和""
                        value = null;
                    }
        ...
        try {
            ...
            for (PropertyDescriptor property : propertyDescriptors) {
                ...
                if (false == key.equals("class")) {
                    ...
                    if (null != value) {
                        map.put(isToUnderlineCase ? CharsetKit.toUnderlineCase(key) : key, value);
                    }
                }
            }
        } catch (Exception e) {
    ...
    public static void copyProperties(Object source, Object target, CopyOptions copyOptions) {
        ...
        if (copyOptions.editable != null) {
            ...
            if (!copyOptions.editable.isInstance(target)) {
                throw new IllegalArgumentException(CharsetKit.format("Target class [{}] not assignable to Editable class [{}]", target.getClass().getName(), copyOptions.editable.getName()));
            }
